...
def lists():
    var_list = ('BIRTHYEAR', 'AGE_T1', 'AGE_T2', 'AGE_T3', 'ZIP_CODE',
       ...
       'HBF_T1', 'HBF_T2', 'MAP_T1', 'MAP_T2', 'SBP_T1', 'SBP_T2', 'CHO_T1', 'GLU_T1', 'CHO_T2', 'GLU_T2',
       'LLDS', 'SUMOFALCOHOL', 'SUMOFKCAL', 'MWK_VAL',
    ...
    groupby_list = ('GENDER','EDUCATION_LOWER_T1',
       ...
       'HTN_MED_T1',
       'RESPIRATORY_DISEASE_T1', 'SMOKING', 'METABOLIC_DISORDER_T1',
...
def widget_hist(data, var_list, groupby_list):
    ...
    widget_groupby_hist = pn.widgets.Select(
        name = 'Groupby', options = list(groupby_list))
    return widget_hist_multi, widget_groupby_hist


def histplot_body(data, widget_hist_multi, widget_groupby_hist):
    histplot = data.hvplot.hist(y = widget_hist_multi, by = widget_groupby_hist)
    return histplot
